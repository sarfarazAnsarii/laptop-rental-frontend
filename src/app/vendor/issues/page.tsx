...
import { Issue, Inventory } from '@/types';
import {
  AlertTriangle, Plus, Monitor, Clock, CheckCircle,
  AlertCircle, Search, MessageSquare, ChevronDown,
} from 'lucide-react';
...
export default function VendorIssuesPage() {
  ...
  // my stock for report modal dropdown
  const [myStock, setMyStock] = useState<Inventory[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.vendor.myIssues(params);
  ...
  async function openReport() {
    // load stock if not loaded
    if (myStock.length === 0) {
      try {
        const res = await api.vendor.stock({ per_page: '100' });
  ...
  return (
    <DashboardLayout>
      <PageHeader
        ...
        action={
          <Button icon={<Plus size={15} />} onClick={openReport}>Report New Issue</Button>
        }
      />
      ...
      <div className="glass-card overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={<AlertTriangle size={22} />}
            ...
            desc={tab === 'all' ? 'Click "Report New Issue" to flag a problem with any laptop' : undefined} />
        ) : (
          <>
            {/* Mobile card list */}
              ...
              {issues.map(issue => {
                ...
                return (
                  <div key={issue.id} className="p-4 space-y-2 animate-fade-in">
                    ...
                    {issue.inventory ? (
                      <div>
                        <div className="text-xs" style={{ color: '#94A3B8' }}>{issue.inventory.brand} {issue.inventory.model_no}</div>
                        <div className="text-xs font-mono" style={{ color: '#475569' }}>{issue.inventory.asset_code}</div>
                      </div>
                    ) : (
                      <div className="text-xs font-mono" style={{ color: '#475569' }}>ID #{issue.inventory_id}</div>
                    )}
                    {issue.admin_notes && (
                    ...
                    <div className="flex justify-end pt-1">
                      <Button variant="ghost" size="sm" icon={<Search size={12} />} onClick={() => setViewIssue(issue)}>View</Button>
                    </div>
                  </div>
                );
              })}
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th className="hidden lg:table-cell">Admin Notes</th>
                    <th></th>
                  </tr>
                </thead>
                <tbody>
                        ...
                        <td>
                          {issue.inventory ? (
                            ...
                            <span className="text-xs font-mono" style={{ color: '#475569' }}>
                              ID #{issue.inventory_id}
                            </span>
                          )}
                        </td>
                        <td>
                          <div className="text-sm font-medium" style={{ color: '#F1F5F9', maxWidth: 220 }}>
                            {issue.title}
                          </div>
                        </td>
            ...
            <select className="inp" value={issueForm.inventory_id}
              ...
              {myStock.map(inv => (
                <option key={inv.id} value={inv.id}>
                  {inv.asset_code} — {inv.brand} {inv.model_no}
                </option>
              ))}
            </select>
          </FormField>
            ...
            {viewIssue.inventory && (
              <div className="flex items-center gap-3 p-3 rounded-xl"
                ...
                <div>
                  ...
                  <div className="text-xs font-mono" style={{ color: '#475569' }}>
                    {viewIssue.inventory.asset_code}
                  </div>
                </div>
              </div>
            )}
