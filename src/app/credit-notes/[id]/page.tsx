      ...
      <div className="animate-fade-in space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2 text-sm" style={{ color: '#475569' }}>
            <Link href="/credit-notes" className="hover:text-blue-600 transition-colors">Credit Notes</Link>
        ...
        <div className="glass-card overflow-hidden">
          <div className="h-1 w-full" style={{ background: resColor }} />
          <div className="p-6">
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0"
                style={{ background: '#FAF5FF', border: '1px solid #E9D5FF' }}>
                <ReceiptText size={24} style={{ color: '#7C3AED' }} />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <h1 className="text-xl font-bold" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#0F172A' }}>Credit Note</h1>
                <p className="text-xs mt-1" style={{ color: '#94A3B8' }}>
                  Issued: {fmtDate(cn.created_at)} · Rental:{' '}
                  <Link href={`/rentals/${cn.rental_id}`} className="hover:underline" style={{ color: '#2563EB' }}>
                    {cn.rental?.rental_no}
                  </Link>
                </p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="glass-card p-5 space-y-3">
            <h2 className="text-xs font-bold uppercase tracking-widest" style={{ color: '#94A3B8', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>Client</h2>
          ...
          <div className="lg:col-span-2 glass-card p-5">
            <h2 className="text-xs font-bold uppercase tracking-widest mb-4" style={{ color: '#94A3B8', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>Credit Breakdown</h2>

            <div className="space-y-0 rounded-xl overflow-hidden" style={{ border: '1px solid #E2E8F0' }}>
              {[
                { label: 'Advance Paid by Client', value: fmt(cn.advance_paid), color: '#0F172A' },
                { label: `Pro-rated Base (${cn.days_used}/${cn.days_in_month} days)`, value: fmt(cn.pro_rated_base), color: '#64748B', sub: true },
                { label: `GST on Pro-rated (${cn.gst_percent || 18}%)`, value: fmt(cn.pro_rated_gst), color: '#64748B', sub: true },
                { label: 'Amount Owed (pro-rated + GST)', value: fmt(cn.pro_rated_total), color: '#B45309' },
              ].map(({ label, value, color, sub }) => (
                <div key={label} className="flex items-center justify-between px-4 py-3"
                  style={{ background: sub ? '#F8FAFC' : 'transparent', borderBottom: '1px solid #E2E8F0' }}>
                  <span className="text-xs" style={{ color: sub ? '#64748B' : '#475569', paddingLeft: sub ? 12 : 0 }}>{label}</span>
                  <span className="text-sm font-semibold" style={{ color }}>{value}</span>
                </div>
              ))}
              <div className="flex items-center justify-between px-4 py-4"
                style={{ background: '#F0FDF4', borderTop: '2px solid #BBF7D0' }}>
                <span className="text-sm font-bold" style={{ color: '#0F172A' }}>Credit Amount (to be returned/adjusted)</span>
                <span className="text-lg font-bold" style={{ color: '#16A34A' }}>{fmt(cn.credit_amount)}</span>
              </div>
            </div>

            {cn.notes && (
              <div className="mt-3 px-3 py-2.5 rounded-xl text-xs" style={{ background: '#F8FAFC', color: '#475569', border: '1px solid #E2E8F0' }}>
                <span style={{ color: '#94A3B8' }}>Notes: </span>{cn.notes}
              </div>
            )}
      ...
      <Modal open={resolveModal} onClose={() => setResolveModal(false)} title="Update Resolution" width="max-w-sm">
        <div className="space-y-4">
          <div className="p-3 rounded-xl text-xs" style={{ background: '#F8FAFC', border: '1px solid #E2E8F0' }}>
            <div className="font-semibold" style={{ color: '#0F172A' }}>{cn?.credit_note_no}</div>
            <div className="mt-1" style={{ color: '#64748B' }}>
              Credit Amount: <span style={{ color: '#16A34A', fontWeight: 700 }}>{fmt(cn?.credit_amount)}</span>
            </div>
          </div>
